	...
	$per_page = 20;
		...
		if($reserve->num_rows) {
			...
			while($row = $reserve->fetch_assoc()) {
				...
				$statuspending = $row['status']=="pending" ? "selected" : "" ;
				$statusconfirmed = $row['status']=="confirmed" ? "selected" : "" ;
				$detail = "";
				...
				$result .=  "<tr>
								...
								<td>
									<select onChange=\"change_stat('".$reserve_id."')\" name='status' id='".$reserve_id."' class='form-control'>
											<option value='pending_$reserve_id' ".$statuspending.">pending</option>
											<option value='confirmed_$reserve_id' ".$statusconfirmed.">confirmed</option>
										</select>
								</td>
							</tr>";
